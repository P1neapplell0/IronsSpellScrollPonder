...
| `gluttony` | Finishes consuming cooked beef. | `GluttonyEffect#finishEating` | The normal Forge item-finish event runs and grants mana. |
...
- `ray_of_siphoning`: the continuous blood beam is user-verified. It uses packet
  index 22 and two origin-relative `Vec3` positions. Recheck its
  `continuous_thrust` start animation after the `MagicData` initialization fix.
- `ray_of_frost`: verify that the complete beam reaches its target. Its visual
  entity stores beam distance in Forge `IEntityAdditionalSpawnData`, not NBT;
  this now travels in the initial projected entity snapshot.
...
- Fireball, fire arrow, and immolate explosions: verify packet index 39 even
  when the packet is sent only to players tracking a projectile or victim.
- Entity-specific spell effects: verify index 40 reaches
  `IClientEventEntity#handleClientEvent` using retained server entity IDs.
- `guiding_bolt`: verify tracking start/stop packets 41 and 42 and the later
  projectile's curved path.
- Heat Surge and Frostwave: verify shockwave packet 43 resolves its particle
  registry ID.
